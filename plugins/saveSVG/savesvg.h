...
class SaveSVG: public SaveSVGInterface, public InterfacePlugin
{
	...
	public slots:
		void ShowWindow(){
			...
			QString header = "Ёкспортировать в SVG...";
			...
			if (SaveFile(lastTo)) {
				QTextCodec::setCodecForCStrings( QTextCodec::codecForName("cp-1251") );
				QMessageBox::information( 0, "SVG", "јнимаци€ успешно сохранена." );
				QTextCodec::setCodecForCStrings( QTextCodec::codecForName("utf8") );
			} else {
				QTextCodec::setCodecForCStrings( QTextCodec::codecForName("cp-1251") );
				QMessageBox::information( 0, "ќшибка!", "Ќевозможно сохранить анимацию." );
				QTextCodec::setCodecForCStrings( QTextCodec::codecForName("utf8") );
			}
		};
		...
		virtual void createPlugin(QObject *parent, QString idParent,plugin::PluginsManager *manager)
		{
			if (idParent == "Main") {
				...
				if(mainWin!=0)
				{
					...
					QTextCodec::setCodecForCStrings( QTextCodec::codecForName("cp-1251") );
					QAction* Export = fileMenu->addAction( "Ёкспортировать в SVG..." );
