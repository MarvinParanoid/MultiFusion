...
class BrushBox: public BrushBoxInterface, public InterfacePlugin
{
	...
	private:
		QObject* brushEditor; /// ”казатель на плагин изменени€ кисти(BrushEditor)

		QDialog dialog; ///ƒиалог в который загружаетс€ плагин изменени€ кисти(BrushEditor)
		QVBoxLayout *l;

		GSRInterface* selection;
		MainWindowInterface* mainWin;/// ”казатель на область рисовани€(PaintWidget)
		PaintWidgetInterface* painter;/// ”казатель на панель инструментов(ToolBox)
